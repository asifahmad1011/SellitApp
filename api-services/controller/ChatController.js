...
module.exports.getAllCommunications = function(matrikel_number,callback){
  var statement = "SELECT chat.receiver_id, users.matrikel_number,users.first_name,users.last_name,ANY_VALUE(products.name) as product_name\
  ...
  GROUP BY users.matrikel_number, chat.receiver_id;";
      sequelize.query(statement).spread((data) => {
          callback(data);
      });
}
